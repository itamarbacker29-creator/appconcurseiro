...
export function ExtrairCargos({ editalId }: { editalId: string }) {
  ...
  async function extrair() {
    setStatus('loading');
    try {
      const r = await fetch(`/api/editais/${editalId}/extrair-cargos`, { method: 'POST' });
      if (r.ok) {
        router.refresh();
      } else {
        setStatus('erro');
  ...
  if (status === 'erro') {
    return (
      <p className="text-[13px] text-(--danger) text-center max-w-xs">
        Não foi possível extrair os cargos. O PDF do edital pode não estar disponível ainda.
      </p>
    );
  }
